...
<head>
	...
	<script>
		...
		function load_goal() {
			...
			var goalId = <?php echo $_GET['id']; ?>;
			var diverId = <?php echo $_GET['diverId']; ?>;
				...
				dataType: "json"
					...
					if(data.skills.length > 0){
						...
						for (var i = 0; i < data.skills.length; i++) {
							$("#insert_skills").append("<div class=\"col-sm-offset-1 col-xs-offset-1\"><p><strong>" + data.skills[i].name + " level " + data.skills[i].level + "</strong><br/>Desired Rating: " + data.skills[i].desiredRating + "<span style='margin-left: 40px;'>Rating: " + data.skills[i].rating + "</p></div><br/>");
						}
					}
					else{
		...
		// Navigates to the edit page for a specific goal
		function edit_goal() {
...
<body>
	<?php include('../common/header.php'); echo_header('View Goal', true, 'diver.php?id=' . $_GET['diverId'], '-sm'); ?>
